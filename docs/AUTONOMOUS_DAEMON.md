...
- Binance 同方向只允许一张 `closePosition=true` 保护，因此改单为撤旧后立即建新；新保护失败或无法读回时立即市价平仓并熔断。该流程不是原子改单，会存在一个 API 往返时长的保护空窗。
...
测试账户清空持仓与所有挂单后，可以运行一次受限闭环探针。它仅允许 `TESTNET`，默认用不超过 50 USDT 名义价值的 ETHUSDT 多单，验证真实成交读回、−7% 服务器保护、撤销旧保护后立即建立 −5% 新保护、主动平仓和测试单清理：
